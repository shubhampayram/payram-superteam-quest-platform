...
export default function App() {
    ...
    async function restoreSession() {
      ...
      if (storedId) {
        try {
          ...
          if (res.ok) { setParticipant(await res.json()); setScreen('campaigns'); }
      ...
      if (storedAdmin) {
        try {
          ...
          if (parsed?.token) { setAdmin(parsed); setScreen('admin_dashboard'); }
  ...
  const handleEnterParticipant = (p: Participant) => {
    ...
    setParticipant(p); setScreen('campaigns');
  };
  const handleLogoutParticipant = () => {
    localStorage.removeItem(PARTICIPANT_KEY); setParticipant(null); setSelectedCampaign(null); setScreen('home');
  };
  const handleSelectCampaign = (camp: Campaign) => { setSelectedCampaign(camp); setScreen('tasks'); };
  const handleAdminSuccess = (adminUser: { id: string; email: string; token: string }) => {
    ...
    setAdmin(adminUser); setScreen('admin_dashboard'); setActiveAdminTab('dashboard');
  };
  const handleAdminLogout = () => {
    localStorage.removeItem(ADMIN_SESSION_KEY); setAdmin(null); setScreen('admin_login');
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-[#0A0A0F] text-zinc-900 dark:text-zinc-100 transition-colors duration-200">
      ...
      {screen === 'home' && <HomeView onEnterPlatform={handleEnterParticipant} onGoToAdmin={() => setScreen('admin_login')} />}

      {screen === 'campaigns' && participant && (
        <CampaignsView participant={participant} onSelectCampaign={handleSelectCampaign} onLogout={handleLogoutParticipant} />
      )}

      {screen === 'tasks' && participant && selectedCampaign && (
        <TasksView participant={participant} campaign={selectedCampaign} onBack={() => setScreen('campaigns')} onLogout={handleLogoutParticipant} />
      )}

      {screen === 'admin_login' && <AdminLogin onLoginSuccess={handleAdminSuccess} onGoBack={() => setScreen('home')} />}
        ...
        <div className="min-h-screen flex flex-col">
            ...
            <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center flex-wrap gap-4">
              <div className="flex items-center space-x-2.5">
                ...
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider text-pink-500 bg-pink-500/10 border border-pink-500/15 px-2 py-0.5 rounded-md mt-0.5">Admin</span>
                </div>
              </div>

              <div className="flex items-center bg-zinc-100 dark:bg-zinc-900/60 p-1 rounded-xl shrink-0 flex-wrap gap-0.5">
                {(
                  [
              ...
              <button onClick={handleAdminLogout}
                className="px-4 py-2 text-xs font-bold bg-[#13131A] dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300 transition-colors flex items-center space-x-1 cursor-pointer">
          ...
          <main className="max-w-7xl w-full mx-auto px-6 py-10 flex-1">
            <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
              {activeAdminTab === 'dashboard'   && <AdminDashboard admin={admin} adminToken={admin.token} />}
              {activeAdminTab === 'campaigns'   && <AdminCampaigns adminToken={admin.token} />}
