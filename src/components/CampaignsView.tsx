  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!campaignsRes.ok || !completionsRes.ok) throw new Error('Failed to load');
    ...
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-[#0A0A0F] text-zinc-900 dark:text-zinc-100 transition-colors duration-200">
      <nav className="border-b border-zinc-200 dark:border-zinc-900 bg-white/80 dark:bg-[#0A0A0F]/80 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center space-x-2 select-none">
            ...
            <span className="font-extrabold text-md tracking-tight bg-gradient-to-r from-zinc-800 to-zinc-950 dark:from-white dark:to-zinc-300 bg-clip-text text-transparent">PayRam Hub</span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <header className="mb-10 text-center md:text-left">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-950 dark:text-white">Active Campaigns</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2 max-w-xl">Choose a campaign, complete tasks, and submit on Superteam to unlock your tokens.</p>
        </header>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white dark:bg-[#13131A] rounded-2xl border border-zinc-200 dark:border-zinc-900 shadow-sm max-w-md mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {campaigns.map((camp) => {
              ...
              const completedCount = completions.filter(c => c.campaign_id === camp.id).length;
              ...
              return (
                <motion.div key={camp.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
                  ...
                  <div className="h-1.5 w-full bg-gradient-to-r from-[#7C3AED] to-pink-500" />
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    <div className="space-y-4">
                      <div>
                        <h3 className="font-extrabold text-xl tracking-tight text-zinc-950 dark:text-white leading-tight">{camp.title}</h3>
                        <div className="flex items-center space-x-1.5 text-zinc-400 mt-2">
                          <Calendar className="w-3.5 h-3.5" />
                          <span className="text-xs font-semibold">{formatDate(camp.start_date)} – {formatDate(camp.end_date)}</span>
                        </div>
                      </div>
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">{camp.description}</p>
                    </div>
                    <div className="mt-6 pt-5 border-t border-zinc-100 dark:border-zinc-900 space-y-3.5">
                      <div className="flex justify-between text-xs font-extrabold uppercase">
                        ...
                        <span className="text-[#7C3AED] dark:text-[#39FF14]">{completedCount}/{totalTasks} Tasks</span>
                      </div>
                      <div className="w-full bg-zinc-100 dark:bg-zinc-900 h-2 rounded-full overflow-hidden">
                        <div className="bg-gradient-to-r from-[#7C3AED] to-pink-500 h-2 rounded-full transition-all duration-300" style={{ width: `${progressPct}%` }} />
                      </div>
                    </div>
                  </div>

                  {isAllCompleted ? (
                    <a href={camp.superteam_submission_url} target="_blank" rel="noreferrer"
                    ...
                    <div className="px-6 pb-6 pt-2">
                      <button onClick={() => onSelectCampaign(camp)}
                        className="w-full bg-[#7C3AED] hover:bg-[#6D28D9] text-white py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center cursor-pointer shadow-lg shadow-[#7C3AED]/10">
                        View Tasks →
                      </button>
                    </div>
                  )}
      ...
      <ErrorReportButton participantId={participant.id} page="campaigns" />
    </div>
  );
}
