...
import { Search, ChevronDown, Check, User, Mail, Globe, Twitter, AlertCircle } from 'lucide-react';
...
export default function HomeView({ onEnterPlatform, onGoToAdmin }: HomeViewProps) {
  ...
  const validate = () => {
    ...
    if (!username.trim()) {
      tempErrors.username = 'Superteam Username is required.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/participants', {
        ...
        body: JSON.stringify({
          superteam_username: username.trim(),
          email: email.trim(),
          country,
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-[#0A0A0F] text-zinc-900 dark:text-zinc-100 transition-colors duration-200">
      ...
      <header className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <button
            id="btn-admin-gate"
            onClick={onGoToAdmin}
            className="text-xs font-semibold px-4 py-2 rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:border dark:border-zinc-800 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 transition-colors pointer-events-auto cursor-pointer"
          >
            Admin Gate Access
          </button>
          <ThemeToggle />
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 md:py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 space-y-6" id="welcome-hero-banner">
          <div className="inline-flex items-center space-x-2 bg-[#7C3AED]/10 border border-[#7C3AED]/20 px-3.5 py-1.5 rounded-full">
            ...
            <span className="text-xs font-extrabold text-[#7C3AED] dark:text-zinc-300 uppercase tracking-widest">
              Live SOL and USDC Quests
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 pt-6 max-w-lg border-t border-zinc-200 dark:border-zinc-800/60">
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-500 font-medium uppercase mt-1">Verified Users</p>
            </div>
            <div>
              <p className="text-2xl md:text-3xl font-black text-pink-500">$50,000</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-500 font-medium uppercase mt-1">Pool Distributed</p>
            </div>
            <div>
              <p className="text-2xl md:text-3xl font-black text-[#7C3AED] dark:text-[#7C3AED]">100%</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-500 font-medium uppercase mt-1">On-Chain Delivery</p>
            </div>
          </div>
        </div>

        {/* Right Side: Participant Entrance Card */}
        <div className="lg:col-span-5">
          <div id="participant-entry-card" className="border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-[#13131A] rounded-3xl p-8 shadow-xl relative overflow-hidden">
            {/* Glowing decorative border elements */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#7C3AED]/20 to-transparent blur-2xl"></div>
            <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-[#39FF14]/5 to-transparent blur-2xl"></div>

            <h2 className="text-2xl font-extrabold mb-2 tracking-tight text-zinc-900 dark:text-white">
              Get Started In Seconds
            </h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">
              No account creation or password required. Use your active Superteam identifier.
            </p>

            {errors.general && (
              <div className="mb-5 p-3.5 bg-red-500/10 border border-red-500/20 text-red-500 dark:text-red-400 rounded-xl text-xs flex items-start space-x-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    id="input-superteam-username"
                    type="text"
                    required
                    placeholder="your-username"
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    className="w-full bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800/80 rounded-xl py-3 pl-11 pr-4 text-sm focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED] transition-all text-zinc-900 dark:text-zinc-100"
                  />
                </div>
                {errors.username && (
              ...
              {/* Field 3: Country (Search Dropdown) */}
              <div className="relative">
                <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-2">
                ...
                <button
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800/80 rounded-xl py-3 px-4 text-left text-sm text-zinc-700 dark:text-zinc-300 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors flex justify-between items-center cursor-pointer"
                >
                  <span className="flex items-center space-x-2">
                    <Globe className="w-4.5 h-4.5 text-zinc-400 flex-shrink-0" />
                    <span>{country ? country : 'Select a Country'}</span>
                  </span>
                  ...
                  <div className="absolute z-30 mt-2 w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl p-2.5 space-y-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg py-1.5 pl-9 pr-3 text-xs text-zinc-900 dark:text-zinc-100 focus:outline-none"
                      />
                    </div>
                      ...
                      {filteredCountries.length === 0 ? (
                        ...
                        filteredCountries.map((item) => (
                          <button
                            ...
                            className="w-full flex items-center justify-between text-xs px-3 py-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-200 text-left transition-colors cursor-pointer"
                          >
              ...
              {/* Field 4: Twitter (Optional) */}
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-2 flex justify-between">
                  <span>Twitter/X Handle <span className="text-zinc-400 font-normal lowercase">(optional)</span></span>
                </label>
                <div className="relative">
                  <Twitter className="absolute left-3.5 top-3.5 w-4.5 h-4.5 text-zinc-400" />
                  <input
              ...
              {/* Submit button */}
              <button
                id="btn-platform-enter"
                type="submit"
                disabled={isSubmitting}
                className="w-full mt-2 bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-extrabold text-sm uppercase tracking-wider py-4 rounded-xl shadow-lg shadow-[#7C3AED]/20 transition-all active:scale-[0.99] flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50"
              >
                {isSubmitting ? (
      ...
      <footer className="border-t border-zinc-200 dark:border-zinc-900 py-6 text-center text-xs text-zinc-400 z-10 relative">
         PayRam Community Quest Hub © 2026. Deployable with Supabase.
      </footer>
    </div>
  );
}
